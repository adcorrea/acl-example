...
public class ForumServiceImpl implements ForumService {
    ...
    public void createPost(Post post) {
        ...
        acl.insertAce(0, BasePermission.ADMINISTRATION, new
                PrincipalSid(
                user.getUsername()), true);

        acl.insertAce(1, BasePermission.DELETE, new
                GrantedAuthoritySid(
                "ROLE_ADMIN"), true);

        if(isAdminUserLogged()){
            acl.insertAce(2, BasePermission.READ, new
                    GrantedAuthoritySid(
                    "ROLE_ADMIN"), true);
        }else{
    ...
    public void deletePost(Post post){
        ObjectIdentity oid = new ObjectIdentityImpl(Post.class,
                post.getId());
